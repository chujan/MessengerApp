...
//  MessengerApp
//
//  Created by Chukwuemeka Jennifer on 15/09/2021.
//
...
class ChatViewController: MessagesViewController {
    ...
    private func presentInPutActionSheet() {
        let actionSheet = UIAlertController(title: "Attach Meadia", message: "what would you like to attach", preferredStyle: .actionSheet)
    ...
    private func presentLocationPicker() {
        ...
        vc.completion = { [weak self] selectedCoorindates in
            ...
            DatabaseManager.shared.sendMessage(to: conversationId, otherUserEmail: strongSelf.otherUserEmail, name: name, newMessage: message, completion: { success in
                if success {
                    print("sent location messages")
                }
                else {
                    print("failed to send location messages")
                }
    ...
    private func listenForMessages(id: String, shouldScrollToBottom: Bool) {
        ...
        DatabaseManager.shared.getAllMessagesForConversations(with:id , completion: { [weak self] result in
            ...
            case .failure(let error):
                print("xfailed to get messages: \(error)")
            }
            ...
            StorageManager.shared.uploadMessagePhoto(with: imageData, fileName: fileName, completion: { [weak self] result in
                ...
                case .success(let urlString):
                  ...
                  guard  let url = URL(string: urlString),
                    ...
                    DatabaseManager.shared.sendMessage(to: conversationId, otherUserEmail: strongSelf.otherUserEmail, name: name, newMessage: message, completion: { success in
                        if success {
                            print("sent photo messages")
                        }
                        else {
                            print("failed to send photo messages")
                        }
...
extension ChatViewController: InputBarAccessoryViewDelegate {
    func inputBar(_ inputBar: InputBarAccessoryView, didPressSendButtonWith text: String) {
        ...
        if isNewConversation {
           // let messages = Message(sender: selfsender, messageId: messageId, sentDate: Date(), kind: .text(text))
